...
RUN apk update && apk upgrade && apk --no-cache add \
    ...
    && cd php-${PHP_VER} \
    && ./configure --enable-mbstring --with-curl=/usr/lib --with-apxs2=/usr/bin/apxs --enable-intl --enable-zip --with-pdo-mysql --with-mysqli --with-openssl --with-curl --with-zlib --with-gd --with-mysqli=mysqlnd --with-pdo-mysql=mysqlnd --with-zlib=shared --enable-opcache\
    && make && make install \
    # Xdebugのインストール
